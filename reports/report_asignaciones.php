<?php
    ...
    $query_bienes = "select a.fecha_asignacion, a.tarjeta_responsable, b.no_clave_control, b.descripcion, b.valor from asignacion a inner join articulo b 
    ...
    $pdf->Cell(0,6,utf8_decode('Área: '.$ae_descripcion), 0, 1, 'L', 0);
    $pdf->Cell(110,6,utf8_decode('Nombre Completo: '.$nombres.$apellidos), 0, 0, 'L', 0);
    $pdf->Cell(110,6,utf8_decode('Creado el: '.$fecha), 0, 1, 'L', 0);
    ...
    $pdf->Cell(80,6, utf8_decode('Descripción'), 0, 1, 'C', 1);
    

    $pdf->SetFont('Arial', '', 12);
    while ($row2 = mysqli_fetch_assoc($result_bienes)) {
        $pdf->Cell(30,6,$row2['fecha_asignacion'], 'T', 0, 'C', 0);
        $pdf->Cell(30,6,$row2['tarjeta_responsable'],'T', 0, 'C', 0);
        $pdf->Cell(25,6,$row2['no_clave_control'], 'T', 0, 'C', 0);
        $pdf->Cell(25,6,'Q '.$row2['valor'], 'T', 0, 'C', 0);
        $pdf->MultiCell(80,6, utf8_decode($row2['descripcion']), 'T', 'J', 0);
        $pdf->Ln(3);
    }
